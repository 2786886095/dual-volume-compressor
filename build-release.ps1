...
Copy-Item -LiteralPath (Join-Path $root 'Assets\*') -Destination (Join-Path $windowsStage 'Assets')
...
Get-ChildItem -LiteralPath (Join-Path $root 'dist') -Recurse -File | Where-Object { $_.Extension -in '.apk', '.zip', '.msix', '.cer', '.gz' } | Get-FileHash -Algorithm SHA256
